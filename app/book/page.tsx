...
export default function BookPage() {
    ...
    const timer=window.setTimeout(()=>{const params = new URLSearchParams(window.location.search);
      ...
      if (draft && !params.get("service")) {
        try {
          ...
          if (saved.customerType === "Home" || saved.customerType === "Business") setCustomerType(saved.customerType);
  ...
  useEffect(() => {
    const timer=window.setTimeout(()=>{if (customerType === "Business" && payment === "Cash") setPayment("M-Pesa");
  ...
  const currentImage = customerType === "Business" ? current.businessImage : current.image;
  ...
  const canContinue = step === 1 || (step === 2 && address.trim() && (customerType === "Home" || company.trim())) || step === 3 || (step === 4 && name.trim() && contact.trim() && (customerType === "Home" || company.trim()));
  ...
  if (complete) return <main className="booking-page"><header className="book-nav"><a className="brand" href="/"><span>M</span>mwenza</a><span>{customerType} booking request</span></header><section className="booking-success"><div className="success-mark">✓</div><small>{customerType.toUpperCase()} REQUEST RECEIVED · {bookingId}</small><h1>You’re all set, {name.split(" ")[0]}.</h1><p>We’ll confirm {customerType === "Business" ? `${company}’s service plan` : "your Mwenza professional"}, final scope and price through {contact}.</p><div className="success-card"><span><small>Service</small><b>{selected[0]}</b></span><span><small>When</small><b>{dates[day].label}, {time}</b></span><span><small>Location</small><b>{address}</b></span><span><small>{customerType === "Business" ? "Frequency" : "Estimated from"}</small><b>{customerType === "Business" ? frequency : `KSh ${total.toLocaleString()}`}</b></span></div><div className="booking-next"><i/><span><b>Request received</b><small>Scope and availability confirmation is next.</small></span><i/><span><b>Professional assigned</b><small>You’ll receive their details before arrival.</small></span></div><div className="success-actions"><a href="/account?view=bookings">Track this booking</a><button onClick={() => {setComplete(false);setStep(1)}}>Book another service</button></div></section></main>;
  ...
  return <main className="booking-page">
    ...
    <div className={`book-shell ${step===1?"book-shell-focus":""}`}>
      ...
      <section className="book-main">
        {step===1 && <><div className="book-heading"><h1>What do you need help with?</h1><p>Choose a service and the option that fits.</p></div><div className="booking-audience"><button aria-pressed={customerType==="Home"} className={customerType==="Home"?"selected":""} onClick={()=>setCustomerType("Home")}><b>For my home</b><small>Household services</small></button><button aria-pressed={customerType==="Business"} className={customerType==="Business"?"selected":""} onClick={()=>setCustomerType("Business")}><b>For my business</b><small>Locations, teams and fleets</small></button></div><div className="book-categories">{serviceKeys.map(key => <button key={key} aria-pressed={service===key} className={service===key?"selected":""} onClick={() => {setService(key);setOption(0);setScope("")}}><img src={customerType==="Business"?catalog[key].businessImage:catalog[key].image} alt={`${customerType} ${catalog[key].name}`}/><span>{catalog[key].name}</span></button>)}</div><h2 className="book-label">Choose an option</h2><div className="book-options">{current.options.map((item,index) => <button key={item[0]} aria-pressed={option===index} className={option===index?"selected":""} onClick={() => setOption(index)}><span><b>{item[0]}</b><small>{customerType==="Business"?"Volume quote":`From KSh ${Number(item[1]).toLocaleString()} · ${item[2]}`}</small></span><i>{option===index?"✓":""}</i></button>)}</div>{((service==="laundry"&&option<2)||(service==="auto-care"&&option===3))&&<div className="book-quantity"><span><b>{service==="laundry"?"Estimated weight":"Number of vehicles"}</b><small>Used for your estimate</small></span><div><button aria-label="Decrease quantity" onClick={() => setQuantity(Math.max(1,quantity-1))}>−</button><b>{quantity} {service==="laundry"?"kg":"vehicles"}</b><button aria-label="Increase quantity" onClick={() => setQuantity(quantity+1)}>+</button></div></div>}<details className="book-optional"><summary>Add job details <span>Optional</span></summary><label className="book-field book-scope">{service==="cleaning"?"Space and rooms":service==="cooking"?"People and dietary needs":service==="fundi"?"What needs attention?":service==="auto-care"?"Vehicle type and condition":service==="laundry"?"Fabric or care preferences":service==="pest-control"?"Pest signs and affected areas":service==="outdoor-care"?"Garden size and tasks":"Support checklist"}<textarea value={scope} onChange={event=>setScope(event.target.value)} placeholder={service==="cleaning"?"e.g. 3-bedroom apartment, 2 bathrooms":service==="cooking"?"e.g. lunch for 8, vegetarian options":service==="fundi"?"Describe the issue, measurements or materials":service==="auto-care"?"e.g. 2 SUVs, interior stains":service==="laundry"?"Delicates, stains, fragrance-free or ironing notes":service==="pest-control"?"e.g. ants near kitchen cabinets, seen for one week":service==="outdoor-care"?"e.g. small lawn, hedge trimming and leaf cleanup":"Items, errands, access and timing"}/></label></details></>}
        {step===2 && <><div className="book-heading"><h1>{customerType==="Business"?"Where will service happen?":"Where should we come?"}</h1><p>{customerType==="Business"?"Add the main location. We’ll confirm any others later.":"Enter the service address."}</p></div>{customerType==="Business"&&<label className="book-field">Business name<input autoFocus value={company} onChange={event=>setCompany(event.target.value)} placeholder="Company or property"/></label>}<label className="book-field">{customerType==="Business"?"Main service location":"Service address"}<input autoFocus={customerType==="Home"} value={address} onChange={event=>setAddress(event.target.value)} placeholder="Estate, street, building or landmark"/></label><div className="address-help"><button onClick={()=>setAddress("Westlands, Nairobi")}>Westlands</button><button onClick={()=>setAddress("Kilimani, Nairobi")}>Kilimani</button><button onClick={()=>setAddress("Karen, Nairobi")}>Karen</button></div><details className="book-optional"><summary>Add access notes <span>Optional</span></summary><label className="book-field">Access and job details<textarea value={instructions} onChange={event=>setInstructions(event.target.value)} placeholder={customerType==="Business"?"Site size, operating hours or access rules":"Gate instructions, parking, stairs or anything else we should know"}/></label></details><p className="book-privacy">Your full address is only shared with the assigned professional.</p></>}
        {step===3 && <><div className="book-heading"><h1>When works for you?</h1><p>{customerType==="Business"?"Set the service frequency, sites and preferred arrival window.":"Choose a day and arrival window."}</p></div>{customerType==="Business"&&<div className="business-booking-controls business-schedule-controls"><label>Frequency<select value={frequency} onChange={event=>setFrequency(event.target.value)}><option>One time</option><option>Weekly</option><option>Twice weekly</option><option>Monthly</option><option>Custom schedule</option></select></label><div><span><b>Locations</b><small>Number of sites</small></span><div><button aria-label="Remove a location" onClick={()=>setLocations(Math.max(1,locations-1))}>−</button><b>{locations}</b><button aria-label="Add a location" onClick={()=>setLocations(locations+1)}>+</button></div></div></div>}<h2 className="book-label">Day</h2><div className="book-days">{dates.map((item,index)=><button key={item.label} aria-pressed={day===index} className={day===index?"selected":""} onClick={()=>setDay(index)}><b>{item.label}</b><small>{item.date}</small></button>)}</div><h2 className="book-label">Arrival window</h2><div className="book-times">{["8–10 AM","10 AM–12 PM","12–2 PM","2–4 PM","4–6 PM"].map(value=><button key={value} aria-pressed={time===value} className={time===value?"selected":""} onClick={()=>setTime(value)}><span>{value}</span>{time===value&&<b>Selected</b>}</button>)}</div></>}
        {step===4 && <><div className="book-heading"><h1>Review your request.</h1><p>{customerType==="Business"?"We’ll confirm scope and volume pricing.":"Nothing is charged until the final details are confirmed."}</p></div><div className="review-box">{customerType==="Business"&&<div><span>Business</span><b>{company}</b><button onClick={()=>setStep(2)}>Edit</button></div>}<div><span>Service</span><b>{selected[0]} · {customerType}</b><button onClick={()=>setStep(1)}>Edit</button></div><div><span>Location</span><b>{address}{customerType==="Business"&&locations>1?` + ${locations-1} more`:""}</b><button onClick={()=>setStep(2)}>Edit</button></div><div><span>Schedule</span><b>{dates[day].label}, {time}{customerType==="Business"?` · ${frequency}`:""}</b><button onClick={()=>setStep(3)}>Edit</button></div></div><div className="contact-grid"><label className="book-field">{customerType==="Business"?"Contact person":"Your name"}<input autoComplete="name" value={name} onChange={event=>setName(event.target.value)} placeholder="Full name"/></label><label className="book-field">Phone or email<input autoComplete="email" inputMode="email" value={contact} onChange={event=>setContact(event.target.value)} placeholder="07… or name@email.com"/></label></div><h2 className="book-label">Payment preference</h2><div className="payment-options">{(customerType==="Business"?["M-Pesa","Card","Invoice"]:["M-Pesa","Card","Cash"]).map(value=><button key={value} aria-pressed={payment===value} className={payment===value?"selected":""} onClick={()=>setPayment(value)}><span>{value==="M-Pesa"?"M":value==="Card"?"▭":value==="Invoice"?"30d":"KSh"}</span><b>{value}</b><i>{payment===value?"✓":""}</i></button>)}</div></>}
        {submitError&&<p className="book-submit-error" role="alert">{submitError}</p>}
      </section>
      {step>1&&<aside className="book-summary"><div className="summary-image"><img src={currentImage} alt={`${customerType} ${current.name}`}/><span>{customerType} · {current.name}</span></div><div className="summary-lines"><span><small>Service</small><b>{selected[0]}</b></span>{customerType==="Business"&&<span><small>Frequency</small><b>{frequency}</b></span>}<span><small>Location</small><b>{address||"Add address"}</b></span><span><small>When</small><b>{step>2?`${dates[day].label}, ${time}`:"Choose a time"}</b></span></div>{instructions&&<p className="summary-note">“{instructions}”</p>}<div className="summary-total"><span><small>{customerType==="Business"?"Pricing":"Estimated from"}</small><b>{customerType==="Business"?"Custom quote":`KSh ${total.toLocaleString()}`}</b></span><p>Confirmed before service.</p></div></aside>}
    </div>
    <footer className="book-footer"><button onClick={()=>step>1?setStep(step-1):history.back()}>{step>1?"← Back":"Cancel"}</button><div><span>{customerType==="Business"?"Business pricing":"Estimated from"} <b>{customerType==="Business"?"Custom quote":`KSh ${total.toLocaleString()}`}</b></span><button disabled={!canContinue||submitting} onClick={next}>{submitting?"Saving request…":step===4?"Request booking →":"Continue →"}</button></div></footer>
  </main>;
}
