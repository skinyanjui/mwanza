...
const allowedReporterTypes = new Set(["customer", "business", "provider"]);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await createNotification({ recipientEmail: email, audience: reporterType === "provider" ? "provider" : reporterType === "business" ? "business" : "customer", bookingId, title: `Issue ${id} received`, message: "Mwenza Operations has received your report and will post resolution updates here." });
...
export async function PATCH(request: Request) {
  ...
  await createNotification({ recipientEmail: current[0].ownerEmail, audience: current[0].reporterType === "provider" ? "provider" : current[0].reporterType === "business" ? "business" : "customer", bookingId: current[0].bookingId, title: `Issue ${id}: ${status}`, message: status === "Resolved" ? "The operations team has marked your report resolved. Contact support if anything remains outstanding." : `Your report is now ${status.toLowerCase()}.` });
