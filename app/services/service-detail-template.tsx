...
export default function ServiceDetailTemplate({ service, segment }: { service: Service; segment: ServiceSegment }) {
  ...
  const headerShell = segment === "business" ? "business" : segment === "government" ? "government" : "home";

  return <main className={`detail-page marketplace-detail${isHome ? " detail-page-lite" : ""}`} data-segment={segment} style={{ "--detail-accent": presentation.accent, "--detail-dark": presentation.dark } as CSSProperties}>
    <SiteHeader shell={headerShell}/>

    <section className="detail-marketplace-head">
      <nav className="detail-breadcrumbs" aria-label="Breadcrumb">
        <Link href="/">Home</Link><span>/</span>{!isHome && <><a href={presentation.returnHref}>{presentation.shortLabel} services</a><span>/</span></>}<b>{current.short}</b>
      </nav>
      {!isHome && (
      ...
      <div className="detail-marketplace-grid">
        ...
        <aside className="detail-booking-card" aria-label={`Request ${presentation.title}`}>
          <small>{isHome ? "STARTING FROM" : "SERVICE PRICING"}</small>
          <div className="detail-booking-price"><strong>{presentation.price}</strong><span>{isHome ? "Scope confirmed first" : "Confirmed after review"}</span></div>
    ...
    <section className="detail-options" id="options">
      ...
      <div className="detail-option-grid">
        {capabilityCards.map((option, index) => <article key={option.name}>
          <small>{isHome ? "OPTION" : "CAPABILITY"}</small>
          <h3>{option.name}</h3>
          <p>{option.description}</p>
          <div><span><small>Pricing</small><b>{option.price}</b></span><span><small>Timing</small><b>{option.timing}</b></span></div>
          <a href={`${presentation.bookingHref}${presentation.bookingHref.includes("?") ? "&" : "?"}option=${index}`}>{isHome ? "Choose this option" : "Add to request"}</a>
        </article>)}
      </div>
    </section>
    ...
    {isHome ? (
      <details className="detail-more">
        <summary>Details, preparation and FAQs</summary>
        ...
        <section className="detail-faq detail-faq-marketplace" id="faq">
          <div><small>COMMON QUESTIONS</small><h2>Good to know.</h2></div>
    ...
    <section className="detail-related">
      ...
      <div>{related.map(item => {
        ...
        return <MarketplaceServiceCard key={item.slug} title={itemPresentation.title} description={itemPresentation.description} price={itemPresentation.price} image={itemPresentation.image} detailHref={`/services/${item.slug}/${segment}`} actionHref={itemPresentation.bookingHref} actionLabel={isHome ? "Book" : "Request"}/>;
      })}</div>
    </section>
    ...
    <SiteFooter/>
    <div className="detail-mobile-bar"><span><small>{isHome ? "From" : "Pricing"}</small><b>{presentation.price}</b></span><a href={presentation.bookingHref}>{presentation.actionLabel}</a></div>
  </main>;
}
