...
type Provider = { id: string; fullName: string; ownerEmail?: string; location: string; services: string; status: string; acceptingWork: number; rating: number; completedJobs: number };
...
export default function OperationsPage() {
  ...
  const operationsPatch = async (body: Record<string, unknown>) => {
    const response = await fetch("/api/operations", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    const data = await response.json();
  ...
  const resolveIncident = async (incident: Incident) => {
    ...
    try {
      const response = await fetch("/api/incidents", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: incident.id, status: "Resolved", assignedTo: "Mwenza Operations" }) });
      const data = await response.json();
  ...
  const activeProviders = providers.filter((item) => item.status === "Active" && item.ownerEmail);
