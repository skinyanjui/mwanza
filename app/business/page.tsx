...
const services = [
  { name: "Commercial cleaning", detail: "Offices, retail, common areas and turnovers.", image: "/business-cleaning.webp" },
  { name: "Linen & uniforms", detail: "Hotels, spas, restaurants and workwear.", image: "/business-linen.webp" },
  { name: "Workplace meals", detail: "Staff meals, meetings and managed meal prep.", image: "/business-cooking.webp" },
  { name: "Facility maintenance", detail: "Plumbing, electrical, carpentry and routine fixes.", image: "/business-fundi.webp" },
  { name: "Fleet washing", detail: "On-site vehicle washing and detailing.", image: "/business-fleet.webp" },
  { name: "Operations support", detail: "Restocking, errands, turnovers and event setup.", image: "/business-support.webp" },
  { name: "Pest management", detail: "Scheduled prevention for offices, hospitality and shared spaces.", image: "/business-pest.webp" },
  { name: "Grounds care", detail: "Routine landscape upkeep for properties and workplaces.", image: "/business-grounds.webp" },
];
...
export default function BusinessPage() {
  ...
  return <main className="biz-page">
    ...
    <section className="biz-services" id="services"><div className="biz-section-head"><small>ONE PARTNER. EIGHT CAPABILITIES.</small><h2>Services that keep work moving.</h2><p>Start with one service or combine several under a single Mwenza account.</p></div><div className="biz-service-grid">{services.map((service,index) => <article key={service.name}><img src={service.image} alt=""/><span>{String(index+1).padStart(2,"0")}</span><div><h3>{service.name}</h3><p>{service.detail}</p><a href="#quote" onClick={() => setSelected([service.name])}>Add to your plan →</a></div></article>)}</div></section>

    <section className="biz-industries" id="industries"><div className="biz-section-head left"><small>MADE FOR THE WAY YOU OPERATE</small><h2>Useful across industries.</h2><p>Mwenza adjusts staffing, timing, supplies and service standards to each location.</p></div><div className="industry-grid">{industries.map((industry,index) => <article key={industry[0]}><span>{String(index+1).padStart(2,"0")}</span><h3>{industry[0]}</h3><p>{industry[1]}</p></article>)}</div></section>

    <section className="biz-how" id="how"><div className="biz-section-head"><small>FROM REQUEST TO ROUTINE</small><h2>Simple to start. Easy to manage.</h2></div><div className="biz-how-grid"><article><span>1</span><h3>Tell us the scope</h3><p>Choose services, locations and the schedule you need.</p></article><article><span>2</span><h3>Walk through the site</h3><p>For larger jobs, we verify requirements before pricing.</p></article><article><span>3</span><h3>Approve one plan</h3><p>Review clear scope, service standards and total pricing.</p></article><article><span>4</span><h3>Track every visit</h3><p>See schedules, completed work, issues and invoices.</p></article></div></section>

    <section className="biz-plans" id="plans"><div className="biz-section-head left"><small>FLEXIBLE SERVICE MODELS</small><h2>Use Mwenza your way.</h2></div><div className="biz-plan-grid"><article><span>ON DEMAND</span><h3>Extra help, when needed.</h3><p>Individual jobs with clear scope and confirmed pricing.</p><ul><li>No contract</li><li>Single location</li><li>Standard scheduling</li></ul><a href="#quote">Request service →</a></article><article className="popular"><span>ROUTINE · MOST POPULAR</span><h3>Your regular service calendar.</h3><p>Reserved weekly or monthly visits with consistent standards.</p><ul><li>Priority scheduling</li><li>Preferred teams</li><li>Consolidated billing</li></ul><a href="#quote">Build a routine →</a></article><article><span>MANAGED</span><h3>One plan across operations.</h3><p>Coordinated services for multiple locations, teams or fleets.</p><ul><li>Dedicated contact</li><li>Custom service levels</li><li>Monthly reporting</li></ul><a href="#quote">Talk to Mwenza →</a></article></div></section>
