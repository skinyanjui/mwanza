...
const services = marketplaceServices.map(service => ({ name: service.businessTitle, detail: service.businessCopy, price: service.businessPrice, image: service.businessImage, slug: service.slug }));
...
export default function BusinessPage() {
  ...
  return <main className="biz-page">
    ...
    <section className="biz-services" id="services"><SectionHeading className="biz-section-head" eyebrow="ONE PARTNER. EIGHT CAPABILITIES." title="Services that keep work moving." description="Start with one service or combine several under a single Mwenza account."/><div className="marketplace-service-grid biz-marketplace-service-grid">{services.map(service => <MarketplaceServiceCard key={service.name} title={service.name} description={service.detail} price={service.price} image={service.image} detailHref={`/services/${service.slug}/business`} actionHref={`/book?service=${service.slug}&audience=business`} actionLabel="Request"/>)}</div></section>
    ...
    <section className="biz-quote" id="quote">
      <div className="biz-quote-copy"><small>BUILD YOUR MWENZA PLAN</small><h2>Tell us what your business needs.</h2><p>We’ll confirm the scope, recommend a schedule and return a clear quote. No commitment.</p><div><span>1</span> Select the services</div><div><span>2</span> Add your operation details</div><div><span>3</span> Receive a tailored plan</div></div>
      <QuickRequestPanel>{submitted ? <QuickRequestSuccess title="Your Mwenza plan is next." description={`A business specialist will contact ${businessName} through ${contact} to verify the details and prepare your quote.`} onReset={() => void setSubmitted(false)}/> : <>
        <QuickRequestHeader/>
        <QuickRequestServices items={services.map(service => service.name)} selected={selected} onToggle={toggle}/>
        <QuickRequestField label="Service frequency"><QuickRequestChoices items={["One time", "Weekly", "Monthly", "Not sure"]} value={frequency} onChange={setFrequency}/></QuickRequestField>
        <QuickRequestRow>
          <QuickRequestField label="Business name"><input required value={businessName} onChange={event => setBusinessName(event.target.value)} placeholder="Company or property" autoComplete="organization"/></QuickRequestField>
          <QuickRequestField label="Number of locations"><input required min="1" value={locationCount} onChange={event => setLocationCount(event.target.value)} placeholder="e.g. 3" inputMode="numeric" type="number"/></QuickRequestField>
        </QuickRequestRow>
        <QuickRequestField label="Work email or phone"><input required value={contact} onChange={event => setContact(event.target.value)} placeholder="How should we reach you?" autoComplete="email"/></QuickRequestField>
        <QuickRequestSubmit ready={requestReady} submitting={submitting} label="Request my service plan" readyNote="No payment required. We verify scope before quoting." incompleteNote="Add your business name, locations and contact details to continue." error={requestError} onSubmit={() => void setSubmitted(true)}/>
      </>}</QuickRequestPanel>
    </section>
